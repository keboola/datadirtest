...
class TestComponent(unittest.TestCase):
    def setUp(self) -> None:
        self.test_datadirs = os.path.join(os.path.dirname(os.path.realpath(__file__)), "resources")

        self.test_datadirs_passing = os.path.join(os.path.dirname(os.path.realpath(__file__)), "resources_passing")
        self.test_datadirs_passing_env = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "resources_passing_env_variables"
        )

        self.test_datadirs_chained = os.path.join(os.path.dirname(os.path.realpath(__file__)), "chained_tests")

        self.test_datadirs_chained_artifacts = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "chained_tests_artifacts"
        )
    ...
    def test_passing_with_env(self):
        tester = DataDirTester(
            self.test_datadirs_passing_env, os.path.join(self.test_datadirs_passing_env, "script.py")
        )
    ...
    def test_context_parameters(self):
        ...
        tester = DataDirTester(
            self.test_datadirs,
            os.path.join(self.test_datadirs, "script.py"),
    ...
    def test_chained_tests_with_artifacts(self):
        tester = DataDirTester(
            self.test_datadirs_chained_artifacts, os.path.join(self.test_datadirs_chained_artifacts, "script.py")
        )
    ...
    def test_chained_tests_with_artifacts_custom(self):
        tester = DataDirTester(
            self.test_datadirs_chained_artifacts,
        ...
        self.assertIn(
            textwrap.dedent("""
            Found artefact in the shared artifacts: artefact-shared.txt
            Found custom artifact: artefact.txt"""),
            output,
        )
